...
| 커뮤니티 관리 기능이 흩어짐 | 공지 · 문의 · 신고 · 관리자 권한 통합 | 사용자와 관리자의 기능을 명확히 구분 |
...
| 대안 | 장점 | 단점 |
| :--- | :--- | :--- |
| 두 서비스를 호출하는 중간 서비스 추가 | 순환 의존성 제거 | 단순 호출 위임만 담당하여 책임이 모호함 |
| 모집글 검증 전용 컴포넌트 분리 | 검증 책임과 서비스 책임이 명확함 | 새로운 컴포넌트가 추가됨 |
...
| 대안 | 장점 | 단점 |
| :--- | :--- | :--- |
| 모집글에 현재 인원 컬럼 저장 | 조회가 단순하고 빠름 | 참여 상태와 인원 컬럼이 불일치할 수 있음 |
| `ACCEPTED` 요청을 실시간 집계 | 실제 상태와 항상 일치 | 조회 시 집계 또는 조인이 필요함 |
