...
public interface UserMapper {
	...
	@Select("""
				SELECT
				     ...
				     , (SELECT NVL(SUM(PB.QUANTITY),0) FROM PLOGUES_USER U JOIN JOIN_REQUEST JR ON(U.USER_ID =JR.USER_ID) JOIN PROOF_BOARD PB ON(JR.JOIN_NO = PB.JOIN_NO) WHERE U.DELETED = 'N' AND U.USER_ID = #{username} AND CATEGORY = 'PLANT') AS TREE_COUNT
